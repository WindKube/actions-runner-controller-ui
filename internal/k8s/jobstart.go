...
// EphemeralRunner and the controller never stamps a timestamp beside it.
//
// Keyed by UID rather than by name on purpose. ARC derives runner pod names from
// the scale set, and those names ARE reused across generations, so a name-keyed
// map would hand a fresh runner the start time of a dead one.
...
// exactly one job, so a cleared jobId means this runner is finished. A nil
// tracker returns the zero time, which lets BuildSnapshot run untracked in tests.
...
//
// Without this the map grows for the life of the process: a runner deleted while
...
// Len reports how many runners are being tracked.
